...
// The test submits a HTTP GET method to the API endpoint and test against the returned response
test('get 1 customer', async t => {
  const customerID = 12345
  const res = await got(`${t.context.baseUrl}/api/customers/${customerID}`, {
    responseType: 'json'
  })
  t.is(res.body.my_message, 'Get One Customer!')
})
